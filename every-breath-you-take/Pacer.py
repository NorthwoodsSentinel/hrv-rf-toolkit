...
class Pacer(QObject):
    ...
    def get_phase_label(self, breathing_rate):
        """Return 'Inhale' or 'Exhale' based on whether the pacer is expanding or contracting."""
        t = time.time()
        # Derivative of sin is cos; positive cos = expanding = inhale
        cos_val = np.cos(2 * np.pi * breathing_rate / 60 * (t - self.phase))
        return "Inhale" if cos_val > 0 else "Exhale"
