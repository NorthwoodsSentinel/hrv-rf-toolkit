...
class HrvAnalyser:
    def __init__(self):
        ...
        self.rmssd_60s_history = HistoryBuffer(500)  # Sliding 60s RMSSD (EliteHRV-comparable)
    ...
    def update_breath_by_breath_metrics(self, t_range):
        ...
        # Sliding 60s RMSSD — comparable to EliteHRV/Whoop HRV score
        t_now = t_range[1]
        ids_60s = np.logical_and(
            self.ibi_history.times > (t_now - 60),
            ~np.isnan(self.ibi_history.values)
        )
        ibi_60s = self.ibi_history.values[ids_60s]
        if len(ibi_60s) >= 4:
            rmssd_60s = np.sqrt(np.mean(np.diff(ibi_60s) ** 2))
            self.rmssd_60s_history.update(t_range[1], rmssd_60s)
